...
def main():
    ...
    for idx, prompt in enumerate(prompts):
        ...
        st.checkbox("Main prompt", value=prompt['is_main'], key=f"main_{prompt_key}")
        ...
        with col3:
            if st.button("Set as Main", key=f"set_main_{prompt_key}"):
                if choose_main_prompt(prompt['name'], access_token):
                    st.success("Main prompt chosen successfully!")
                    st.rerun()
                else:
    ...
    with st.form("new_prompt"):
        ...
        submit_new_prompt = st.form_submit_button("Create New Prompt")
        new_is_main = st.checkbox("Set as Main Prompt")
        ...
        if submit_new_prompt:
            ...
            if success:
                ...
                st.error(f"Failed to create prompt. {msg}")
